...
class AgronomicEngine {
  ...
  static double calculateDailyGDD(
    CropProfile profile,
    double dailyMaxTemp,
    double dailyMinTemp,
  ) {
    // Cap max temperature
    double maxT = dailyMaxTemp > profile.maxTemperatureGDD
        ? profile.maxTemperatureGDD
        : dailyMaxTemp;
    // Cap min temperature
    double minT = dailyMinTemp < profile.baseTemperature
  ...
  static double calculateETc(
    ...
    double windFactor =
        1 + ((current.windSpeed ?? 5.0) / 100.0); // Simple multiplier
  ...
  static List<String> assessDiseaseRisk(
    CropProfile profile,
    List<DailyForecast> forecast5Days,
  ) {
    List<String> alerts = [];
    ...
    if (profile.susceptibleToHighHumidity) {
      ...
      for (var day in forecast5Days) {
        if ((day.precipitationProbability ?? 0.0) > 0.5 &&
            (day.tempMax ?? 0) >= 15 &&
            (day.tempMax ?? 0) <= 28) {
        ...
        if (consecutiveHighRiskDays >= 2) {
          alerts.add(
            'High risk of fungal disease (e.g. Blight/Rust) due to consecutive humid, warm days. Consider preventative fungicide.',
          );
    ...
    if (profile.susceptibleToFrost) {
      for (var day in forecast5Days) {
        if ((day.tempMin ?? 10) <= 2) {
          alerts.add(
            'Frost warning! Temperatures expected to drop to ${day.tempMin}°C. Cover crops if possible.',
          );
          break;
        }
      }
  ...
  static Map<String, double> calculateNutrientDepletion(
    CropProfile profile,
    double totalYieldKg,
  ) {
